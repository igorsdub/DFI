%INVBKBT=importdata('/Users/Taisong/Research/Rebekka/allostery/1atmCR8/tetramer_struct2_MD/4DXMmwcovarmat.dat');
...
for k=1:1       %k is the dimension of direction
    ...
    for j=1:resnum          % perturb j
        ...
        delforce(3*j1-2:3*j1)=direct;
        j2=j1+resnum0;
        delforce(3*j2-2:3*j2)=direct;
        j3=j2+resnum0;
        delforce(3*j3-2:3*j3)=direct;
        j4=j3+resnum0;
        delforce(3*j4-2:3*j4)=direct;
...
%            end
    ...
    allcolumnsum(:,k)=sum(delRperbDistMat(1:resnum,1:resnum,k),2);
    bindcolumnsum(:,k)=sum(delRperbDistMat(1:resnum,resperturb(1:npb),k),2);
    ratio(:,k)=(bindcolumnsum(:,k)/npb)./(allcolumnsum(:,k)/(resnum-1));
end
% 
...
% arr_raw=arr;
% resid_raw=vertcat(resid,resid,resid,resid);
% arr=zeros(length(resid),1);
% for i=1:length(resid)
%     idx=find(resid_raw==resid(i));
%     if ~isempty(idx)
%         arr(i)=mean(arr_raw(idx));
% %        arr(i)=max(arr_raw(idx));
%     end
% end
% 
