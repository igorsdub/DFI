...
C############################################################
	PARAMETER (NR=828,GAMMA=10.0,nbG=1)
C############################################################
...
C############################################################
	OPEN(50,FILE='4DXM_C1_struct_atmorder.pdb')
	OPEN(51,FILE='4DXM_C1_struct_atmorder.pdb')
...
c ********************
	...
	open(13,file='4DXM.BindRatioTop_all_directions.txt')
	open(14,file='4DXM.BindRatioBottom_all_directions.txt')
...
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2)-2,3*(nperturbRes+RESNUM2)-2
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2*2)-2,3*(nperturbRes+RESNUM2*2)-2
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2*3)-2,3*(nperturbRes+RESNUM2*3)-2
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		write(*,*) nperturbRes,i 
...
c--- this is the denominator of the Equation 10 in Equation 10 in Gerek and Ozkan, Plos Comp Biol,2010
	   ...
	   sum1 = 0.0
		...
		ENDDO
		write(28,*) i, sum1	
		allSumColumn_X(i)=sum1/(RESNUM-1)
	   ENDDO
...
c- this part is the numerator part of Equation 10 in Gerek and Ozkan, Plos Comp Biol, 20011 paper
	   ...
	   sum = 0.0
		...
		ENDDO
		write(27,*) i, sum	
		bindSumRow_X(i) = sum/noPeptideSize
	   ENDDO
...
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2)-1,3*(nperturbRes+RESNUM2)-1
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2*2)-1,3*(nperturbRes+RESNUM2*2)-1
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2*3)-1,3*(nperturbRes+RESNUM2*3)-1
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
...
1001	enddo
	   ...
	   sum1 = 0.0
		...
		ENDDO
		write(32,*) i, sum1	
		allSumColumn_Y(i)=sum1/(RESNUM-1)
	   ENDDO
...
C############################################################
	   ...
	   sum = 0.0
		...
		ENDDO
		write(31,*) i, sum	
		bindSumRow_Y(i) = sum/noPeptideSize
	   ENDDO
...
C			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2),3*(nperturbRes+RESNUM2)
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
...
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2*3),3*(nperturbRes+RESNUM2*3)
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
c		write(*,*) nperturbRes,i,delForce(i)
...
1002	enddo
	   ...
	   sum1 = 0.0
		...
		ENDDO
		write(36,*) i, sum1	
		allSumColumn_Z(i)=sum1/(RESNUM-1)
	   ENDDO
...
C############################################################
	   ...
	   sum = 0.0
		...
		ENDDO
		write(35,*) i, sum	
		bindSumRow_Z(i) = sum/noPeptideSize
	   ENDDO
...
C			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2)-2,3*(nperturbRes+RESNUM2)-1
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2*2)-2,3*(nperturbRes+RESNUM2*2)-1
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2*3)-2,3*(nperturbRes+RESNUM2*3)-1
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
c		write(*,*) nperturbRes,i,delForce(i)
...
1003	enddo
	   ...
	   sum1 = 0.0
		...
		ENDDO
		write(40,*) i, sum1	
		allSumColumn_XY(i)=sum1/(RESNUM-1)
	   ENDDO
...
C############################################################
	   ...
	   sum = 0.0
		...
		ENDDO
		write(39,*) i, sum	
		bindSumRow_XY(i) = sum/noPeptideSize
	   ENDDO
...
1004	enddo
	   ...
	   sum1 = 0.0
		...
		ENDDO
		write(44,*) i, sum1	
		allSumColumn_XZ(i)=sum1/(RESNUM-1)
	   ENDDO
...
C############################################################
	   ...
	   sum = 0.0
		...
		ENDDO
		write(43,*) i, sum	
		bindSumRow_XZ(i) = sum/noPeptideSize
	   ENDDO
...
C			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2)-1,3*(nperturbRes+RESNUM2)
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2*2)-1,3*(nperturbRes+RESNUM2*2)
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2*3)-1,3*(nperturbRes+RESNUM2*3)
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
c		write(*,*) nperturbRes,i,delForce(i)
...
1005	enddo
	   ...
	   sum1 = 0.0
		...
		ENDDO
		write(48,*) i, sum1	
		allSumColumn_YZ(i)=sum1/(RESNUM-1)
	   ENDDO
...
C############################################################
	   ...
	   sum = 0.0
		...
		ENDDO
		write(47,*) i, sum	
		bindSumRow_YZ(i) = sum/noPeptideSize
	   ENDDO
...
C			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2)-2,3*(nperturbRes+RESNUM2)
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2*2)-2,3*(nperturbRes+RESNUM2*2)
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
		ENDDO
		DO i=3*(nperturbRes+RESNUM2*3)-2,3*(nperturbRes+RESNUM2*3)
c			delForce(i) = DELF*(ran1(idum)*2-1)
c			delForceRand(i) = delForce(i)
			delForce(i) = 1.0
c		write(*,*) nperturbRes,i,delForce(i)
...
1006	enddo
	   ...
	   sum1 = 0.0
		...
		ENDDO
		write(55,*) i, sum1	
		allSumColumn_XYZ(i)=sum1/(RESNUM-1)
	   ENDDO
...
C############################################################
	   ...
	   sum = 0.0
		...
		ENDDO
		write(54,*) i, sum	
		bindSumRow_XYZ(i) = sum/noPeptideSize
	   ENDDO
...
c each direction and take the maximum of these values here:
	  ...
	  DO i=1,RESNUM
		...
		write(13,122) i,bindSumRow_X(i),bindSumRow_Y(i),bindSumRow_Z(i),bindSumRow_XY(i),bindSumRow_XZ(i),bindSumRow_YZ(i),bindSumRow_XYZ(i) 
		write(14,122) i,allSumColumn_X(i),allSumColumn_Y(i),allSumColumn_Z(i),allSumColumn_XY(i),allSumColumn_XZ(i),allSumColumn_YZ(i),allSumColumn_XYZ(i)
