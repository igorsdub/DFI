...
c------------Changes you need to run this program:
c			 (1) You need to change NR value given in PARAMETER for your protein
c			 (2) replace 4-letters pdb id (here 2fg4 so replace it to your 4-letter pdb id
c			 (3) Be sure that 0.current.pdb is your input file and it is under your working folder
c			 (4) Be sure that 1YMZmwcovarmat.dat file obtained from ptraj analysis of MD trajectories is
c				 copied to your working folder
c			 (5) be sure to change the number in input.parms file that matches with NR value
c			 
c			 to compile the code: g77 -o run prs-MDNew.f 
c			 to execute the code: ./run
c	
c			 program will generate "1YMZ-dfi.dat" so theser are dfi values corresponding to each residue
c
c		  
c-----------------------------------------------------------------------
...
C   Input file for covariance matrix if MD data is used
C	OPEN(81,FILE='1YMZmwcovarmat.dat')
...
c	OPEN (95,file='20sloweigenx.txt')
c	OPEN (96,file='20sloweigeny.txt')
c	OPEN (97,file='20sloweigenz.txt')
c	OPEN (98,file='20sloweigenr.txt')

	
	open(88,file='1YMZ-dfi.dat')
	open(89,file='1YMZ-S2-Max.dat')
	open(90,file='1YMZ-S1-Avg.dat')
	open(91,file='1YMZ-S1-Max.dat')
        open(99,file='1YMZ_INVBKBT.txt')

	open(15,file='1YMZ-ResponseVector-Magnitude-Avg.dat')
	open(16,file='1YMZ-ResponseVector-Magnitude-Max.dat')
...
c      enddo
		...
		do i=1,resnum
				write(82,128) (delRperbDistMat_Avg(i,j),j=1,resnum)
		enddo
128	format(500f8.5)
