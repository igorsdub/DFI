...
#    bfac,relbfac,pctbfac,zscorebfac = dfianal(bfac,Array=True)
    ...
    with open('dfianalysis.csv','w') as outfile:
        ...
        for i in range(len(dfi)):
            outfile.write("%d,%s,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n"%(i,ATOMS[i].res_name,
               dfi[i],reldfi[i],pctdfi[i],zscoredfi[i],mdfi[i],relmdfi[i],pctmdfi[i],zscoremdfi[i],hmdfi[i],relhmdfi[i],pcthmdfi[i])
